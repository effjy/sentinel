...
static char       g_icon[64] = "sentinel";
...
static void fill_props(GVariantBuilder *b, int id) {
    ...
    const char *label = (id == ID_SHOW) ? "Show Sentinel"
                      : (id == ID_QUIT) ? "Quit Sentinel" : "";
...
static void menu_method(GDBusConnection *, const char *, const char *,
    ...
    if (g_strcmp0(method, "GetLayout") == 0) {
        ...
        if (g_strcmp0(name, "label") == 0)
            val = g_variant_new_string(id == ID_SHOW ? "Show Sentinel"
                                     : id == ID_QUIT ? "Quit Sentinel" : "");
...
static GVariant *sni_get_prop(GDBusConnection *, const char *, const char *,
    ...
    if (g_strcmp0(prop, "Id") == 0)            return g_variant_new_string("sentinel");
    if (g_strcmp0(prop, "Title") == 0)         return g_variant_new_string("Sentinel");
    ...
    if (g_strcmp0(prop, "ToolTip") == 0) {
        ...
        return g_variant_new("(sa(iiay)ss)", g_icon, &icon, "Sentinel",
                             "Real-time machine guardian");
    }
    return nullptr;
}
