// firewall.cpp — Sentinel's per-process outbound firewall page (ported from
// Warden's GUI). It owns the single Unix-socket connection to sentinel-daemon
// and is the I/O hub for the whole GUI:
//
//   * firewall traffic (ASK / EVENT) drives the connection-request prompt and
//     the live activity log on this page, and the Rules tab, and
//   * posture frames (PBEGIN/PFINDING/PEND/PALERT) that arrive on the same
//     socket are forwarded verbatim to the Posture page via posture_on_line().
//
// The Posture page sends control lines back to the daemon (e.g. "SCANNOW")
// through daemon_send(), which writes on this page's socket.
//
...
// ---------------------------------------------------------------------------
// Page state (one instance for the unified app)
// ---------------------------------------------------------------------------
struct App {
    GtkWidget *window      = nullptr;   // top-level, for transient dialogs
...
bool daemon_send(const char *line) {
    if (g_fw.sock_fd < 0) return false;
    send_line(&g_fw, line);
    return true;
}
...
// ---------------------------------------------------------------------------
// Rules view — read /etc/sentinel/rules.conf directly (root-owned, readable)
// ---------------------------------------------------------------------------
...
static void show_prompt(App *app, guint id, const std::string &proto, const std::string &pid,
    ...
    if (app->window && gtk_widget_get_visible(app->window))
        gtk_window_set_transient_for(GTK_WINDOW(dlg), GTK_WINDOW(app->window));
    gtk_window_set_modal(GTK_WINDOW(dlg), app->window && gtk_widget_get_visible(app->window));
    gtk_window_set_default_size(GTK_WINDOW(dlg), 460, -1);
    gtk_window_set_icon_name(GTK_WINDOW(dlg), "sentinel");
...
static void dispatch(App *app, const std::string &line) {
    // Posture frames share this socket; hand them straight to the Posture page.
    if (line.compare(0, 7, "PBEGIN\t") == 0 || line.compare(0, 9, "PFINDING\t") == 0 ||
        line == "PEND" || line.compare(0, 7, "PALERT\t") == 0) {
        posture_on_line(line.c_str());
        return;
    }
    ...
    if (f[0] == "HELLO") {
        set_status(app, "Connected to sentinel-daemon", "ok");
...
dropped:
    ...
    app->sock_fd = -1;
    app->io_source = 0;
    set_status(app, "sentinel-daemon not running — retrying…", "bad");
...
// ---------------------------------------------------------------------------
// Page-specific CSS (component classes only; the shell sets the app background)
// ---------------------------------------------------------------------------
static void apply_firewall_css(void) {
    static const char *CSS =
        ".fw-title { font-weight: bold; font-size: 16px; color: #7aa2f7; }"
    ...
    gtk_style_context_add_provider_for_display(
        ...
        GTK_STYLE_PROVIDER_PRIORITY_APPLICATION + 1);   // after the shell theme
    g_object_unref(p);
}
...
GtkWidget *firewall_build(GtkWindow *parent) {
    App *app = &g_fw;
    app->window = GTK_WIDGET(parent);
    apply_firewall_css();

    GtkWidget *root = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);

    // Header row: title + live connection status.
    GtkWidget *header = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 12);
    gtk_widget_set_margin_top(header, 10); gtk_widget_set_margin_bottom(header, 6);
    ...
    GtkWidget *title = gtk_label_new("OUTBOUND FIREWALL");
    gtk_label_set_xalign(GTK_LABEL(title), 0.0);
    gtk_widget_add_css_class(title, "fw-title");
    GtkWidget *sub = gtk_label_new("approve each program's outgoing connections");
    ...
    reload_rules(app);
    log_append(app, "Firewall ready. Waiting for connection attempts…");
